...
def hybrid_retrieve(query, retrievers, weights, k=3, rrf_k=60):
    ...
    # self Algorithm for Reciprocal Rank Fusion - This did what ensemble retiever did with langchain which they removed from SDK
...
class HybridRetriever:
    ...
    #BM25 doesnt support incremental updates so you need to rebuild when updating documents
    def add_documents(self, documents: List[Document]):
        """Add new documents to both retrievers"""
        # Add to vector store
        self.vectorstore.add_documents(documents)

        """ BM25 is a statistical term‑frequency model.
            It needs to compute global statistics over all documents, such as:
            1. IDF (Inverse Document Frequency) 2. term frequencies 3.document lengths 4.average document length
            These values change whenever you add new documents.Because BM25 does not maintain a dynamic index, 
            it cannot simply “add” new docs.It must recompute the entire index.
        """
